...
namespace hj
{
	...
	public:
		...
		virtual void Create();
		virtual void Spawn(Vector2 pos, Vector2 dir);

		virtual void OnCollisionEnter(class Collider* other) override;

		virtual void SetState(GameObject::eState type);
		Status GetStat() { return stat; }
		void SetStat(float power, Vector2 dir) { stat.power = power; stat.direction = dir; }

		void SetWeapon(GameObject* weapon) { mOwner = weapon; }
		GameObject* GetWeapon() { return mOwner; }

		void Attack() { SetState(GameObject::eState::Pause); }

		Empty* GetEmpty() { return empty; }

	private:
		GameObject* mOwner;
		Empty* empty;
		Status stat;
		eBulletState mBstate;
	};
